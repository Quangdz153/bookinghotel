...
public class EditRoomType extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        String amentities = request.getParameter("amenities");
        
        
        double price = 0 , area =0;
        int capacity = 0 , typeID = 0;
        try{
            price = Double.parseDouble(price_raw);
            area = Double.parseDouble(area_raw);
            capacity = Integer.parseInt(capacity_raw);
            typeID = Integer.parseInt(typeID_raw);
        }catch(NumberFormatException e){
            System.out.println(e);
        }
        new DAO.RoomTypeDAO().updateTypeID(new RoomType(typeID, typeName, description, 
                price, capacity, image, area, amentities));
        
        request.getRequestDispatcher("roomtypecontrol").forward(request, response);
    } 
