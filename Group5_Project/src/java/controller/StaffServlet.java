...
//        List<Staff> sortedList = null;
        ...
        if (searchstaff != null && !searchstaff.isEmpty()) {
            list = dao.searchStaff(searchstaff, sortby, isDescending, index);
            count = dao.countSearchResults(searchstaff);
            System.out.println("search count: " + count);
            System.out.println("Success: " + list.size() + "..." + searchstaff);
        } else {
//            list = dao.getAllStaff();
            list = dao.sortStaff(sortby, isDescending, index);
            count = dao.getTotalSatff();
...
//        Staff staffAccount = dao.getStaffByAccountID(acID);
//        HttpSession session = request.getSession();
//        session.setAttribute("acc", staffAccount); // account là đối tượng chứa thông tin nhân viên đăng nhập

        request.setAttribute("staffID", staffID);
//        request.setAttribute("acID", acID);
        request.setAttribute("searchstaff", searchstaff);

        request.setAttribute("listPage1", list);
...
//        request.setAttribute("staffAccount", staffAccount);
        request.getRequestDispatcher("staffCRUD.jsp").forward(request, response);
    }
